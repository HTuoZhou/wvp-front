  ...
  <div style="width: 100%">
    <div class="page-header">
      <div class="page-title">
        <el-page-header content="国标录像" @back="goBack"></el-page-header>
      </div>
    </div>
    <el-container>
      <el-aside width="300px">
        <div class="record-list-box-box">
          <el-date-picker v-model="chooseDate" placeholder="日期" size="mini" type="date" value-format="yyyy-MM-dd"
                          @change="dateChange()"></el-date-picker>
          <div v-loading="recordsLoading" :style="recordListStyle" class="record-list-box">
            <ul v-if="detailFiles.length >0" class="infinite-list record-list">
              <li v-for="item in detailFiles" class="infinite-list-item record-list-item">
                ...
                <i class="el-icon-download" style="color: #409EFF;margin-left: 5px;" @click="downloadRecord(item)"></i>
              </li>
            </ul>
          </div>
          <div v-if="detailFiles.length ==0" class="record-list-no-val" size="mini">暂无数据</div>
        </div>

      </el-aside>
      <el-main style="padding-bottom: 10px;">
        <div :style="playerStyle" class="playBox">
          <player ref="recordVideoPlayer"
                  ...
                  :videoUrl="videoUrl"
                  autoplay
                  fluent live style="max-height: 100%"></player>
        </div>
        <div class="player-option-box">
          <div>
            ...
            <el-button-group>
              <el-button class="iconfont icon-zanting" size="mini" title="开始" @click="gbPause()"></el-button>
              <el-button class="iconfont icon-kaishi" size="mini" title="暂停" @click="gbPlay()"></el-button>
              ...
              <el-button class="iconfont icon-xiazai1" size="mini" title="下载选定录像"
                         @click="downloadRecord()"></el-button>
              <el-button v-if="sliderMIn === 0 && sliderMax === 86400" class="iconfont icon-slider" size="mini"
                         title="放大滑块" @click="setSliderFit()"></el-button>
              <el-button v-if="sliderMIn !== 0 || sliderMax !== 86400" class="iconfont icon-slider-right" size="mini"
