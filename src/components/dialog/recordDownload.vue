  ...
  <div id="recordDownload">
    <el-dialog v-if="showDialog" :append-to-body="true" :close-on-click-modal="false" :destroy-on-close="true"
      ...
      <el-row>
        ...
        <el-col :span="6">
          <el-button v-if="percentage < 100" icon="el-icon-download" size="mini"
                     title="点击下载可将以缓存部分下载到本地" @click="download()">停止缓存并下载
          </el-button>
          <el-button v-if="downloadFile" icon="el-icon-download" size="mini" title="点击下载"
                     @click="downloadFileClientEvent()">点击下载
          </el-button>
        </el-col>
      </el-row>
    </el-dialog>
