<template>
  <div id="devicePlayer" v-loading="isLoging">
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="showVideoDialog" title="视频播放" top="0" @close="close()">
      <div style="width: 100%; height: 100%">
        <el-tabs v-if="Object.keys(this.player).length > 1" v-model="activePlayer" :stretch="true" type="card"
          ...
          <el-tab-pane label="WebRTC" name="webRTC">
            <rtc-player v-if="activePlayer === 'webRTC'" ref="webRTC" :error="videoError"
                        :hasAudio="hasAudio" :message="videoError" :videoUrl="videoUrl" :visible.sync="showVideoDialog"
                        autoplay fluent height="100px" live></rtc-player>
          </el-tab-pane>
      ...
      <div id="shared" style="text-align: right; margin-top: 1rem;">
        <el-tabs v-model="tabActiveName" @tab-click="tabHandleClick">
          <el-tab-pane label="实时视频" name="media">
            <div style="display: flex; margin-bottom: 0.5rem; height: 2.5rem;">
              ...
              <el-input v-model="getPlayerShared.sharedUrl" :disabled="true">
                <template slot="append">
                  <i v-clipboard="getPlayerShared.sharedUrl" class="cpoy-btn el-icon-document-copy" title="点击拷贝"
            ...
            <div style="display: flex; margin-bottom: 0.5rem; height: 2.5rem;">
              ...
              <el-input v-model="getPlayerShared.sharedIframe" :disabled="true">
                <template slot="append">
                  <i v-clipboard="getPlayerShared.sharedIframe" class="cpoy-btn el-icon-document-copy" title="点击拷贝"
                     @success="$message({type:'success', message:'成功拷贝到粘贴板'})"></i>
                </template>
              </el-input>
            </div>
            <div style="display: flex; margin-bottom: 0.5rem; height: 2.5rem;">
              ...
              <el-input v-model="getPlayerShared.sharedRtmp" :disabled="true">
                <el-button slot="append" v-clipboard="getPlayerShared.sharedRtmp" icon="el-icon-document-copy"
                           title="点击拷贝"
            ...
            <div style="display: flex; justify-content: left;">
              ...
              <div class="control-panel">
                <el-button-group>
                  ...
                  <el-input-number v-model="presetPos" :max="255"
                  ...
                  <el-input-number v-model="cruisingSpeed" :max="4095"
                  ...
                  <el-input-number v-model="cruisingTime" :max="4095"
                                   :min="1" :precision="0" controls-position="right" size="mini"
                  ...
                  <el-input-number v-model="cruisingGroup" :max="255"
                  ...
                  <el-input-number v-model="scanSpeed" :max="4095"
                                   :min="1" :precision="0" controls-position="right" size="mini"
                  ...
                  <el-input-number v-model="scanGroup" :max="255"
          ...
          <el-tab-pane v-loading="tracksLoading" label="编码信息" name="codec">
            <p>
              无法播放或者没有声音?&nbsp&nbsp&nbsp试一试&nbsp
              <el-button v-if="!coverPlaying" size="mini" type="primary" @click="coverPlay">转码播放</el-button>
              <el-button v-if="coverPlaying" size="mini" type="danger" @click="convertStopClick">停止转码</el-button>
            </p>
            <div class="trank">
              <p v-if="tracksNotLoaded" style="text-align: center;padding-top: 3rem;">暂无数据</p>
              <div v-for="(item, index) in tracks" loading style="width: 50%; float: left">
                <span>流 {{ index }}</span>
                <div v-if="item.codec_type == 0" class="trankInfo">
                  <p>格式: {{ item.codec_id_name }}</p>
                  <p>类型: 视频</p>
                  <p>分辨率: {{ item.width }} x {{ item.height }}</p>
                  <p>帧率: {{ item.fps }}</p>
                </div>
                <div v-if="item.codec_type == 1" class="trankInfo">
                  <p>格式: {{ item.codec_id_name }}</p>
                  <p>类型: 音频</p>
