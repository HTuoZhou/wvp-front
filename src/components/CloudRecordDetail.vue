    ...
    <el-container>
      <el-aside width="260px">
        <div class="record-list-box-box">
          <div style="margin-top: 20px">
            <el-date-picker v-model="chooseDate" :picker-options="pickerOptions" placeholder="日期" size="mini"
                            style="width: 160px" type="date" value-format="yyyy-MM-dd"
                            @change="dateChange()"></el-date-picker>
            <el-button icon="fa fa-cloud-download" size="mini" style="margin: auto; margin-left: 12px " title="裁剪合并"
                       type="primary" @click="drawerOpen"></el-button>
          </div>
          <div :style="recordListStyle" class="record-list-box">
            <ul v-if="detailFiles.length >0" v-infinite-scroll="infiniteScroll" class="infinite-list record-list">
              <li v-for="(item,index) in detailFiles" :key="index" class="infinite-list-item record-list-item">
                ...
                <el-tag v-if="choosedFile == item" type="danger">
                  <i class="el-icon-video-camera"></i>
                  {{ item.substring(0, 17) }}
                </el-tag>
                <a
      ...
      <el-main style="padding: 22px">
        <div :style="playerStyle" class="playBox">
          <player ref="recordVideoPlayer" :height="true" :videoUrl="videoUrl" style="width: 100%"></player>
        </div>
      ...
      <div class="drawer-box">
        ...
        <el-tabs v-model="tabVal" style="height: 100%" type="border-card" @tab-click="tabClick">
          <el-tab-pane name="running">
            ...
            <ul class="task-list">
              <li v-for="(item,index) in taskListForRuning" :key="index" class="task-list-item">
          ...
          <el-tab-pane name="ended">
            ...
            <ul class="task-list">
              <li v-for="(item, index) in taskListEnded" :key="index" class="task-list-item">
                <div class="task-list-item-box" style="height: 2rem;line-height: 2rem;">
                  <span>{{ item.startTime.substr(10) }}-{{ item.endTime.substr(10) }}</span>
                  <a :href="getFileBasePath()  + '/download.html?url=download/' + item.recordFile"
                     class="el-icon-download download-btn" target="_blank">
                  </a>
                </div>
              </li>
