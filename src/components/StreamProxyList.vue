  ...
  <div id="streamProxyList" style="width: 100%">
    <div class="page-header">
      ...
      <div class="page-header-btn">
        <el-button icon="el-icon-plus" size="mini" style="margin-right: 1rem;" type="primary" @click="addStreamProxy">
          添加代理
        </el-button>
        <el-button v-if="false" icon="el-icon-search" size="mini" style="margin-right: 1rem;" type="primary"
                   @click="addOnvif">搜索ONVIF
        </el-button>
        <el-button circle icon="el-icon-refresh-right" size="mini" @click="refresh()"></el-button>
      </div>
    </div>
    <devicePlayer ref="devicePlayer"></devicePlayer>
    <el-table :data="streamProxyList" :height="winHeight" style="width: 100%">
      <el-table-column label="名称" min-width="120" prop="name" show-overflow-tooltip/>
      <el-table-column label="流应用名" min-width="120" prop="app" show-overflow-tooltip/>
      <el-table-column label="流ID" min-width="120" prop="stream" show-overflow-tooltip/>
      <el-table-column label="流地址" min-width="400" show-overflow-tooltip>
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">

            <el-tag v-if="scope.row.type == 'default'" size="medium">
              <i v-clipboard="scope.row.url" class="cpoy-btn el-icon-document-copy" title="点击拷贝"
                 @success="$message({type:'success', message:'成功拷贝到粘贴板'})"></i>
              {{ scope.row.url }}
            </el-tag>
            <el-tag v-if="scope.row.type != 'default'" size="medium">
              <i v-clipboard="scope.row.srcUrl" class="cpoy-btn el-icon-document-copy" title="点击拷贝"
                 @success="$message({type:'success', message:'成功拷贝到粘贴板'})"></i>
              {{ scope.row.srcUrl }}
            </el-tag>
          </div>
      ...
      <el-table-column label="流媒体" min-width="180" prop="mediaServerId"></el-table-column>
      ...
      <el-table-column label="国标编码" min-width="180" prop="gbId" show-overflow-tooltip/>
      ...
      <el-table-column label="创建时间" min-width="150" prop="createTime" show-overflow-tooltip/>
      <el-table-column label="音频" min-width="120">
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">
            <el-tag v-if="scope.row.enableAudio" size="medium">已启用</el-tag>
            <el-tag v-if="!scope.row.enableAudio" size="medium" type="info">未启用</el-tag>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="录制" min-width="120">
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">
            <el-tag v-if="scope.row.enableMp4" size="medium">已启用</el-tag>
            <el-tag v-if="!scope.row.enableMp4" size="medium" type="info">未启用</el-tag>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="无人观看" min-width="160">
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">
            <el-tag v-if="scope.row.enableRemoveNoneReader" size="medium">移除</el-tag>
            <el-tag v-if="scope.row.enableDisableNoneReader" size="medium">停用</el-tag>
            <el-tag v-if="!scope.row.enableRemoveNoneReader && !scope.row.enableDisableNoneReader" size="medium"
                    type="info">不做处理
            </el-tag>
          </div>
        </template>
      ...
      <el-table-column fixed="right" label="操作" width="360">
        <template slot-scope="scope">
          <el-button v-if="scope.row.enable" icon="el-icon-video-play" size="medium" type="text"
                     @click="play(scope.row)">播放
          </el-button>
          <el-divider direction="vertical"></el-divider>
          <el-button v-if="scope.row.enable" icon="el-icon-switch-button" size="medium" type="text"
                     @click="stop(scope.row)">停用
          </el-button>
          <el-divider direction="vertical"></el-divider>
          <el-button v-if="!scope.row.enable" :loading="scope.row.startBtnLoading" icon="el-icon-check" size="medium"
                     type="text" @click="start(scope.row)">启用
          </el-button>
          ...
          <el-button icon="el-icon-delete" size="medium" style="color: #f56c6c" type="text"
                     @click="deleteStreamProxy(scope.row)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
